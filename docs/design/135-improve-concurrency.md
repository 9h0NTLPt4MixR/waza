...
- Add `internal/execution/sdkclient.go` exposing `SharedClient(opts) (CopilotClient, error)` guarded by `sync.Once`.
...
- `prompt_grader.gradeIndependent` and `runPairwiseOnce` switch to the shared client and **stop calling `client.Stop()`** (sessions are still created/disconnected per call).
- Lifetime: started lazily on first use; stopped from `cmd_run.go`'s existing top-level `engine.Shutdown()` path (or from a single `execution.ShutdownShared(ctx)` if grading happens after engine shutdown).
