...
- **Full**: Skill has an `eval.yaml` with tasks and at least 2 distinct grader types.
- **Partial**: Skill has an `eval.yaml` but fewer than 2 grader types or no tasks.
- **Missing**: No `eval.yaml` found for the skill.
