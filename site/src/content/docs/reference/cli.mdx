...
|------|-------|------|---------|-------------|
| `--context-dir` | | string | `./fixtures` | Fixtures directory path |
| `--output` | `-o` | string | | Save results JSON to file |
| `--output-dir` | | string | | Save output artifacts to directory |
...
| `--suggest` | | bool | false | Generate a Copilot report suggesting skill improvements based on test outcomes |
| `--update-snapshots` | | bool | false | Update or create `diff` grader snapshot files to match current workspace output |
| `--discover` | | bool | false | Walk directory tree to discover and run all skill evals |
...
| `--no-summary` | | bool | false | Skip writing combined summary.json for multi-skill runs |
| `--skip-graders` | | bool | false | Skip grading (execution only); use with `waza grade` to grade later |
| `--interpret` | | bool | false | Print a plain-language interpretation of the results |
...
| `--skills-dir` | Skills directory (overrides detection and defaults) |
| `--evals-dir` | Evals directory (overrides detection and defaults) |
| `--results-dir` | Results directory (overrides detection and defaults) |
...
| `--output-dir` | `-d` | Directory to scaffold into (default: current directory) |
...
|------|---------|-------------|
| `--target` | `medium-high` | Target level: `low`, `medium`, `medium-high`, `high` (iterative mode only) |
| `--max-iterations` | `5` | Max improvement loops (iterative mode only) |
| `--auto` | | Auto-apply without prompting (iterative mode only) |
| `--all` | | Process all skills in workspace |
| `--filter` | | Filter skills by adherence level (used with `--all`): `low`, `medium`, `medium-high`, `high` |
| `--copilot` | | Generate a non-interactive markdown report with Copilot suggestions |
| `--model` | `claude-sonnet-4-20250514` | Model to use with `--copilot` |
| `--scaffold-triggers` | | Generate `tests/trigger_tests.yaml` from SKILL.md USE FOR / DO NOT USE FOR phrases |
...
Run graders against existing results without re-executing an agent.

```bash
waza grade <eval.yaml>
```

Use `waza run --skip-graders --output results.json` to capture execution output, then
`waza grade` to apply graders separately — useful when graders need to be updated or
re-run without a full agent execution.

### Arguments

| Argument | Description |
|----------|-------------|
| `<eval.yaml>` | Path to the evaluation spec file |

### Flags

| Flag | Short | Description |
|------|-------|-------------|
| `--results` | | Path to `waza run` JSON output **(required)** |
| `--task` | | Task ID to grade (omit to grade all tasks) |
| `--workspace` | | Agent workspace directory for file-based graders (default: `.`) |
| `--judge-model` | | Model override for prompt graders |
| `--output` | `-o` | Write full `EvaluationOutcome` JSON to file |
...
waza grade eval.yaml --results results.json --task my-task

# Save graded results to a new file
waza grade eval.yaml --results results.json -o graded.json
```

## waza cache

Manage the evaluation result cache.

```bash
waza cache <subcommand>
```
...
| Flag | Default | Description |
|------|---------|-------------|
| `--cache-dir` | `.waza-cache` | Cache directory to clear |

#### Examples

```bash
waza cache clear
waza cache clear --cache-dir /tmp/my-cache
```

## waza session

View and manage session event logs written during eval runs.

```bash
waza session <subcommand>
```

### waza session list

List recorded session log files.
...
| Flag | Default | Description |
|------|---------|-------------|
| `--dir` | `.` | Directory to search for session logs |

#### Examples

```bash
waza session list
waza session list --dir ./session-logs
```

### waza session view

Display a session timeline from an NDJSON session log file.
...
| Argument | Description |
|----------|-------------|
| `<session-file>` | Path to the session log file |

#### Examples

```bash
waza session view session-2024-01-01.ndjson
```
...
| `--http` | | Start HTTP dashboard server (default when `--tcp` is not set) |
| `--no-browser` | | Don't auto-open the browser |
| `--results-dir` | `.` | Directory to read results from |
| `--tcp` | | TCP address for JSON-RPC (e.g., `:9000`) |
| `--tcp-allow-remote` | | Allow binding to non-loopback addresses (WARNING: no authentication) |
...
waza serve --no-browser           # skip auto-open
waza serve --tcp :9000            # JSON-RPC TCP server
waza serve --tcp :9000 --tcp-allow-remote  # expose to network
waza serve --results-dir ./results # load results from a specific dir
```
