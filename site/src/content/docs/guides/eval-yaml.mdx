...
OpenAI Evals `modelgraded` specs usually collapse into Waza's `prompt` grader. The judge prompt carries the label semantics, while Waza handles execution and scoring; in independent mode the final score can be fractional, and in pairwise mode it reflects the winner selection.

| OpenAI Evals field | Waza equivalent | Notes |
| ------------------ | --------------- | ----- |
| `prompt` | `graders[].config.prompt` | Put the judging instructions directly in the prompt |
| `choice_strings` | prompt text | List the labels in the judge prompt and tell the judge how to map each label to pass/fail |
| `choice_scores` | prompt text | Encode the scoring rule in the judge prompt; use `pairwise` mode when the comparison is relative |
| `input_outputs` | task files under `tasks/` | Turn each example into one Waza task file with its own `inputs.prompt` and expected checks |
...
OpenAI's registry uses this pattern for fixed-choice factual classification. In Waza, keep the evaluation as a single prompt grader and put each example in its own task file:
  ...
  - type: prompt
    ...
    config:
      ...
      prompt: |
        ...
        Call set_pairwise_winner with winner="skill" if the skill run wins.
        Call set_pairwise_winner with winner="baseline" if the baseline run wins.
        Use winner="tie" if neither answer is clearly better.
