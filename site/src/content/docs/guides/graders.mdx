...
<Tabs>
  <TabItem label="Fragment checking">
    ```yaml - type: diff name: api_updates config: expected_files: - path:
    "src/api.py" contains: - "+from fastapi import FastAPI" -
    "+@app.get('/health')" - "-import flask" ```
  </TabItem>
  <TabItem label="Snapshot matching">
    ```yaml - type: diff name: exact_config config: expected_files: - path:
    "config.json" snapshot: "snapshots/expected_config.json" ```
  </TabItem>
</Tabs>
...
| `threshold`  | string | Score threshold between `0.0` and `1.0`. Default is `0.6`.                   |
